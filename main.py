...
class HangmanAPI(object):
    ...
    def guess(self, word):  # word input example: "_ p p _ e "
        ...
        ###############################################
        policy_net = Network()
        policy_net.load_weights('policy.h5')
        player = NNAgent(policy_net)
        player.guessed = self.guessed_letters
        player.eval()
        clean_word = word[::2]
        guess = player.select_action(clean_word)
...
api = HangmanAPI(access_token="1305e3976a8742a88b64f11d4f10d1", timeout=2000)
